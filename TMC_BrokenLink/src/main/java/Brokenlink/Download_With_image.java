...
public class Download_With_image {
		...
		for (int i = 0; i < arrayList.size(); i++) {
			...
			for (int j = 0; j < list.size(); j++) {
				...
				// String link2=link1.replace("https://thanecity.gov.in/tmc/cache/1/", "");
				 ...
				 //System.out.println("===="+list.get(j).getAttribute("data-id"));
				 if (list.get(j).getAttribute("data-id")!=null) {
				  String img[] = list.get(j).getAttribute("data-id").split(",");
				  for (int l = 0; l < img.length; l++) {
					  
					  System.out.println("https://thanecity.gov.in/tmc/cache/"+img[l]);
					  String imagepath="https://thanecity.gov.in/tmc/cache/"+img[l];
					  String imagepathlink="https://thanecity.gov.in/tmc/cache/"+img[l];
					  String pdfname[] = imagepath.replace("https://thanecity.gov.in/tmc/cache/1/", "").split("/"); 
				  ...
				  System.out.println(Parentpath+path); 
						  ...
						  File file = new File(Parentpath+path); file.mkdirs();
						  
						  DownloadPDFFromURL(imagepathlink.replaceAll(" ", "%20"), Parentpath+path);
